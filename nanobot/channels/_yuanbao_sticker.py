...
def get_sticker_by_name(name: str) -> Optional[dict]:
    """Look up a sticker by name, with fuzzy matching fallback.

    Match priority:
      1. Exact match (name)
      2. Name contains query / query contains name
      3. Description contains query
      4. Fuzzy scoring (same algorithm as sticker-search)

    Returns sticker dict or None.
    """
    if not name:
        return None
...
def get_random_sticker(category: str | None = None) -> dict:
    """Return a random sticker, optionally filtered by category keyword."""
    if category:
        candidates = [
...
def get_sticker_by_id(sticker_id: str) -> Optional[dict]:
    """Look up a sticker by sticker_id."""
    if not sticker_id:
        return None
...
def search_stickers(query: str, limit: int = 10) -> list[dict]:
    """Fuzzy search stickers, return top N results sorted by relevance.

    Scoring combines name/description substring match, character multiset
    coverage, bigram Jaccard, and subsequence ratio. Name is weighted slightly
    above description.
    """
    ...
    for sticker in STICKER_MAP.values():
        ...
        scored.append((max(name_s, desc_s), sticker))
...
def build_face_msg_body(
    ...
    """Build a TIMFaceElem message body.

    Yuanbao convention:
      - index is always 0 (the server identifies the sticker via data field)
      - data is a JSON string with sticker_id / package_id etc.

    Args:
        face_index: Reserved, currently unused in wire format (Yuanbao uses index=0).
        face_type: Reserved (compatibility, unused).
        data: Pre-serialised JSON string. If None, only index is sent.
...
def build_sticker_msg_body(sticker: dict) -> list:
    """Build a TIMFaceElem message body from a STICKER_MAP entry."""
